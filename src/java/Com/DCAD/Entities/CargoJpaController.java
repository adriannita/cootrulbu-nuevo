    ...
    public void create(Cargo cargo) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Usuario usuarioListUsuario : cargo.getUsuarioList()) {
                Cargo oldCargoidCargoOfUsuarioListUsuario = usuarioListUsuario.getCargoidCargo();
                usuarioListUsuario.setCargoidCargo(cargo);
                usuarioListUsuario = em.merge(usuarioListUsuario);
                if (oldCargoidCargoOfUsuarioListUsuario != null) {
                    oldCargoidCargoOfUsuarioListUsuario.getUsuarioList().remove(usuarioListUsuario);
                    oldCargoidCargoOfUsuarioListUsuario = em.merge(oldCargoidCargoOfUsuarioListUsuario);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Usuario usuarioListOldUsuario : usuarioListOld) {
                if (!usuarioListNew.contains(usuarioListOldUsuario)) {
                    ...
                    illegalOrphanMessages.add("You must retain Usuario " + usuarioListOldUsuario + " since its cargoidCargo field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Usuario usuarioListNewUsuario : usuarioListNew) {
                if (!usuarioListOld.contains(usuarioListNewUsuario)) {
                    Cargo oldCargoidCargoOfUsuarioListNewUsuario = usuarioListNewUsuario.getCargoidCargo();
                    usuarioListNewUsuario.setCargoidCargo(cargo);
                    usuarioListNewUsuario = em.merge(usuarioListNewUsuario);
                    if (oldCargoidCargoOfUsuarioListNewUsuario != null && !oldCargoidCargoOfUsuarioListNewUsuario.equals(cargo)) {
                        oldCargoidCargoOfUsuarioListNewUsuario.getUsuarioList().remove(usuarioListNewUsuario);
                        oldCargoidCargoOfUsuarioListNewUsuario = em.merge(oldCargoidCargoOfUsuarioListNewUsuario);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Usuario usuarioListOrphanCheckUsuario : usuarioListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Cargo (" + cargo + ") cannot be destroyed since the Usuario " + usuarioListOrphanCheckUsuario + " in its usuarioList field has a non-nullable cargoidCargo field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
